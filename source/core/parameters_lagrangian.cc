...
namespace Parameters::Lagrangian
{
  ...
  LagrangianPhysicalProperties::declareDefaultEntry(ParameterHandler &prm)
  {
    prm.declare_entry("size distribution type",
                      "uniform",
                      Patterns::Selection(
                        "uniform|normal|log-normal|histogram"),
                      "Particle size distribution"
                      "Choices are <uniform|normal|log-normal|histogram>.");
    ...
    prm.declare_entry("diameters value list",
                      "0.001",
                      Patterns::List(Patterns::Double()),
                      "List of diameter values");
    prm.declare_entry("diameters probability list",
                      "1.",
                      Patterns::List(Patterns::Double()),
                      "List of diameter probabilities");
    ...
    ParameterHandler   &prm)
  {
    ...
    if (size_distribution_type_str == "uniform")
      ...
      {
        size_distribution_type = size_distribution_type::histogram;
        particle_list_diameter.at(particle_type) =
          entry_string_to_vector(prm, "diameters value list");
        particle_list_probability.at(particle_type) =
          entry_string_to_vector(prm, "diameters probability list");

        double probability_sum =
          std::reduce(particle_list_probability.at(particle_type).begin(),
                      particle_list_probability.at(particle_type).end());

        if (probability_sum != 1.0)
          {
            for (double &i : particle_list_probability.at(particle_type))
              {
                i = i / probability_sum;
              }
          }
    ...
    std::unordered_map<unsigned int, double> &surface_energy)
  {
    for (unsigned int counter = 0; counter < particle_type_maximum_number;
         ++counter)
      {
        ...
        particle_list_diameter.insert({counter, std::vector<double>{0.}});
        particle_list_probability.insert({counter, std::vector<double>{1.}});
